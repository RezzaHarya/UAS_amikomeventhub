...
@section('content')
    ...
    <section class="max-w-7xl mx-auto px-6 py-20 flex flex-col md:flex-row items-center gap-12">
        ...
        <div class="flex-1 relative">
            ...
            <div
                class="absolute -bottom-6 -left-6 glass p-6 rounded-2xl shadow-xl z-20 border border-white bg-white/80 backdrop-blur">
    ...
    <section class="max-w-7xl mx-auto px-6 pt-10">
        ...
        <div class="flex flex-wrap gap-4">
            @foreach($categories as $category)
                <div
                    class="px-6 py-3 bg-white border border-slate-200 rounded-2xl shadow-sm hover:border-indigo-600 hover:text-indigo-600 hover:shadow-md transition cursor-pointer font-bold text-slate-700">
                    {{ $category->name }}
                </div>
            @endforeach
        </div>
    ...
    <section id="events" class="max-w-7xl mx-auto px-6 py-20">
        <div class="flex justify-between items-end mb-12">
            ...
            <div class="flex gap-2">
                <button
                    class="p-3 border rounded-xl hover:bg-white hover:shadow-md transition font-bold text-slate-600">Semua
                    Event</button>
            </div>
        </div>
        ...
        <div class="flex flex-wrap justify-center gap-8 md:gap-12 items-center">
            @foreach($partners as $partner)
                <div class="group flex flex-col items-center gap-3">
                    <div
                        class="w-32 h-16 md:w-40 md:h-20 bg-white border border-slate-100 rounded-2xl p-4 flex items-center justify-center hover:shadow-lg hover:border-indigo-200 transition duration-300">
                        @if($partner->logo_url)
                            <img src="{{ asset('storage/' . $partner->logo_url) }}" alt="{{ $partner->name }}"
                                class="max-h-full max-w-full object-contain filter grayscale group-hover:grayscale-0 transition duration-300">
                        @else
                            <span
                                class="text-slate-400 font-bold text-xs tracking-wider uppercase text-center">{{ $partner->name }}</span>
                        @endif
                    </div>
